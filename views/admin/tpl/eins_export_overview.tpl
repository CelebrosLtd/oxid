            ...
            [{foreach from=$aPlugins key="sPluginId" item="oPlugin" name="plugin"}]
                <div class="well">
                    <form action="[{$oViewConf->getSelfLink()}]" method="post" id="csvForm_[{$smarty.foreach.plugin.index}]" enctype="multipart/form-data">
                        <div class="row-fluid">
                            <div class="span5">
                                <h2>[{$oPlugin->getTitle()}]</h2>
                            </div>
                            <input type="hidden" name="cl" value="eins_export_overview" />
                    ...
                    function finalizeExport(form) {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                $('#wrap').load('[{$oViewConf->getSelfLink}]?cl=eins_export_overview&stoken=[{$sToken}]&shp=[{$sShopId}] #inner_wrap');
                            }
                        });
                    }
