...
//        $sSearchParam = utf8_encode($sSearchParam);
        ...
        $sInitialSearchStr = $sSearchStr = rawurldecode($sSearchParam);
...
//        $sSearchHandle = $qsr->SearchInformation->SessionId;
...
//        $qsr->Questions = $this->_setLeadQuestion($qsr->Questions);
        ...
        $this->_aViewData['sCelAnalyticsType'] = 'search';
    ...
    protected function _setRecommendedMessage($sRecommendedMessage) {
        ...
        $sRecommendedMessage = utf8_encode($sRecommendedMessage);
//        $sRecommendedMessage = str_replace(array("#%", "%#"), array("<b>", "</b>"), $sRecommendedMessage);
//        $sRecommendedMessage = utf8_decode($sRecommendedMessage);
//        var_dump($sRecommendedMessage);
     ...
     */
    public function ReplaceExtendedChars($sValue, $blReverse = false) {   // we need to replace this for compatibility with XHTML
    ...
    public function setNrOfCatArticles($articlesCount)
    {
        ...
        for ($i = 0; $i < count($arrNrOfCatArticles); $i++) {
            ...
            if ($iPageSize > $articlesCount && $i != 0)
                break;
            $res[] = $iPageSize;
        }
...
//        var_dump($aSearchPath->Count);
        $link = "";

        $iStartIndex = 0;
        if (sizeof($aSearchPath) > 0) {
            foreach ($aSearchPath as $oSearchPath) {
//                var_dump($oSearchPath);
...
//print_r($propertiesBag->custom_message);die;
    ...
    protected function _isCampaignActive($propertiesBag)
    {
        ...
        if (isset($propertiesBag->end_datetime) && $propertiesBag->end_datetime != "9999/12/31 11:59:59 PM" && strtotime($propertiesBag->end_datetime) < time())
            $bActiveCampain = false;
    ...
    public function answerIsInSearchPath($oThisAnswer) {
        if (!$this->_aViewData['aSearchPath']) {
            return false;
        }

        $aSearchPath = $this->_aViewData['aSearchPath']->Items;
        foreach ($aSearchPath as $oItem) {
            $oAnswer = $oItem->Answers->Items;
            if ($oAnswer[0]->Id == $oThisAnswer->Id) {
                return true;
            }
        }
